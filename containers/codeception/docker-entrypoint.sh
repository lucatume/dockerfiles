...
test "${XDEBUG_DISABLE:-0}" == 1 && {
  echo -n "Disabling XDebug extension ..."
...
if [ "0" == "${SKIP_DB_CHECK}" ]; then
  if [ -n "${CHECK_DB_HOST}" ] && [ -n "${CHECK_DB_NAME}" ] && [ -n "${CHECK_DB_USER}" ]; then
    timeout=0
    timeout_message="\n\033[031mWordPress database not available: check db credentials or the WordPress container health.\033[0m"
...
if [ "0" == "${SKIP_URL_CHECK}" ]; then
  if [ -n "${CHECK_URL}" ]; then
    echo -n "Waiting for WordPress site to be available at ${CHECK_URL} ..."
    if [ "$(curl -Lkf --retry-connrefused --retry 15 --retry-delay 2 -o /dev/null --stderr /dev/null "${CHECK_URL}")" ]; then
      echo -e "\n\033[31mWordPress site at ${CHECK_URL} not available: check db credentials or the WordPress container health.\033[0m"
      exit 1
    fi
    echo -e "\033[32m done\033[0m"
  fi
